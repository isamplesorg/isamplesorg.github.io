...
ok() { printf "  ok   %s\n" "$1"; }
no() { printf "  FAIL %s (%s)\n" "$1" "$2"; fail=1; }
hdr() { curl -s --max-time 20 -A "$UA" -I "$@" | grep -i "^$H:" | head -1 | tr -d '\r' | cut -d' ' -f2-; }

UUID_RE='^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
current_version() {
  # The ACTIVE deployment is the last block wrangler prints; a split/gradual
  # deployment prints more than one "(NN%)" line in that block. Fail closed:
  # print nothing unless there is exactly one 100% version in the last block.
  local out; out=$(npx wrangler deployments list -c wrangler.toml 2>&1) || { echo "!! wrangler deployments list failed:" >&2; echo "$out" | tail -3 >&2; return 1; }
  local last; last=$(echo "$out" | awk '/^Created:/{blk=""} {blk=blk"\n"$0} END{print blk}')
  local ids; ids=$(echo "$last" | grep -oE '\([0-9]+%\) [0-9a-f-]{36}')
  [ "$(echo "$ids" | grep -c .)" = "1" ] || { echo "!! active deployment is split or unparsable:" >&2; echo "$ids" >&2; return 1; }
  echo "$ids" | grep -q '^(100%)' || { echo "!! active deployment is not at 100%: $ids" >&2; return 1; }
  echo "$ids" | awk '{print $2}'
}
...
verify() {
  ...
  local S CR CL
  S=$(curl -s --max-time 20 -A "$UA" -o /dev/null -w '%{http_code}' -I -H 'Range: bytes=0-' "$ORIGIN/$PROBE_FILE")
  ...
  for R in 'bytes=0-99' 'bytes=100-199' 'bytes=-100'; do
    S=$(curl -s --max-time 20 -A "$UA" -o /dev/null -w '%{http_code}' -I -H "Range: $R" "$ORIGIN/$PROBE_FILE")
    [ "$S" = "200" ] && ok "HEAD '$R' -> 200 (shim has not widened)" || no "HEAD '$R' -> 200" "got $S"
  done
  S=$(curl -s --max-time 20 -A "$UA" -o /dev/null -w '%{http_code}' -I "$ORIGIN/$PROBE_FILE")
  ...
  S=$(curl -s --max-time 20 -A "$UA" -o /dev/null -w '%{http_code}' -H 'Range: bytes=0-99' "$ORIGIN/$PROBE_FILE")
  [ "$S" = "206" ] && ok "ranged GET -> 206" || no "ranged GET -> 206" "got $S"
  local N; N=$(curl -s --max-time 20 -A "$UA" -o /dev/null -w '%{size_download}' -H 'Range: bytes=0-99' "$ORIGIN/$PROBE_FILE")
  [ "$N" = "100" ] && ok "ranged GET body = 100 bytes" || no "ranged GET body" "got $N"
  S=$(curl -s --max-time 20 -A "$UA" -o /dev/null -w '%{http_code}' "$ORIGIN/$PROBE_FILE")
  ...
  S=$(curl -s --max-time 20 -A "$UA" -o /dev/null -w '%{http_code}' -I "$ORIGIN/$BIG_FILE")
  [ "$S" = "200" ] && ok "boot-critical file present ($BIG_FILE)" || no "boot-critical file" "got $S"
  S=$(curl -s --max-time 20 -A "$UA" -o /dev/null -w '%{http_code}' -I "$ORIGIN/current/wide.parquet")
...
# --- preflight --------------------------------------------------------------------
PREV=$(current_version) || exit 1
echo "$PREV" | grep -qE "$UUID_RE" || { echo "!! Could not determine the active production version ('$PREV'). Refusing to deploy without a rollback target."; exit 1; }
echo "==> Production Worker 'isamples-data' — currently active version: $PREV"
...
if ! npx wrangler deploy -c wrangler.toml | tee /tmp/prod_deploy.log; then
  echo
  echo "!! wrangler deploy reported failure. That does NOT prove production is unchanged"
  echo "   (the upload can activate before wrangler/tee report an error). Re-querying:"
  NOW=$(current_version) || NOW=""
  echo "   active version now: ${NOW:-UNKNOWN}   (was $PREV)"
  if [ -n "$NOW" ] && [ "$NOW" = "$PREV" ]; then
    echo "   Same version as before: production unchanged."
    verify no || echo "   ...but verification of the previous version FAILED — investigate before anything else."
  else
    echo "   Version changed or unknown: treat as a partial deploy. Verifying for the shim:"
    verify yes || echo "   Verification failed."
    echo "   Roll back if in doubt:   $0 --rollback $PREV"
  fi
  exit 1
fi
NEW=$(current_version) || NEW=""
echo "==> Active version now: ${NEW:-UNKNOWN}  (was $PREV)"
[ -n "$NEW" ] && [ "$NEW" != "$PREV" ] || echo "!! Active version did not change — the deploy may not have taken. Verification below will tell."
...
echo "Rollback if anything looks wrong:  $0 --rollback $PREV"
echo "───────────────────────────────────────────────────────────────"
