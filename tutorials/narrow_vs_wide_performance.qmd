...
|--------|-------------|-----------|-----------|---------|
| **Narrow** | Stores relationships as separate edge rows (`otype='_edge_'`) | ~860 MB | 101,387,180 rows | All 4 sources |
| **Wide** | Stores relationships as `p__*` columns on entity rows | ~292 MB | ~20.7M rows | All 4 sources |
| **Wide + H3** | Wide format + pre-computed H3 spatial indices (`h3_res4/6/8`) | ~307 MB | ~20.7M rows | All 4 sources |
...
<p><em>The wide schema has ~80% fewer rows because edge rows are eliminated. The H3 variant adds 3 spatial index columns (h3_res4/6/8).</em></p>
</div>
...
<tbody>
<tr><td style="text-align: left;"><strong>Full scan (292MB)</strong></td>
...
<tr style="border-bottom: 1px solid #155724;">
<td style="padding: 8px;">Full scan (292MB wide)</td>
...
<li><strong>File sizes:</strong> Narrow ~860MB, Wide ~292MB, Wide+H3 ~307MB</li>
...
| **Memory limits** | 860 MB narrow file may stress browser memory; wide format is safer |
...
The wide format eliminates ~81M edge rows (~66% file size reduction: 860 MB → 292 MB). The H3 variant adds ~15MB for spatial index columns that enable cell-based geospatial filtering.
