...
| **Wide** | Stores relationships as `p__*` columns on entity rows | ~292 MB | ~20M rows | All 4 sources |
| **Wide + H3** | Wide format + pre-computed H3 spatial indices (`h3_res4/6/8`) | ~307 MB | ~20M rows | All 4 sources |
...
The wide format eliminates ~81M edge rows (~66% file size reduction: 860 MB → 292 MB). The H3 variant adds ~14MB for spatial index columns that enable cell-based geospatial filtering.
