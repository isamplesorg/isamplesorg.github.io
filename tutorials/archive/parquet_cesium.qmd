...
**Query Strategy (EVENT_SAMPLE_LOCATION + EVENT_SAMPLING_SITE)**:
- Starts at a GeospatialCoordLocation (clicked point)
- Walks **backward** via `EVENT_SAMPLE_LOCATION` edges to find SamplingEvents that reference this location
- From those events, finds MaterialSampleRecords produced by them via `MSR_PRODUCED_BY`
- Requires site context (INNER JOIN on `EVENT_SAMPLING_SITE` → SamplingSite) - this is the provenance constraint
...
## Understanding Location Traversals Using Typed Edges
...
| Edge Type | From → To |
|-----------|-----------|
| `MSR_PRODUCED_BY` | MaterialSampleRecord → SamplingEvent |
| `MSR_REGISTRANT` | MaterialSampleRecord → Agent |
| `MSR_KEYWORDS` | MaterialSampleRecord → IdentifiedConcept |
| `MSR_HAS_CONTEXT_CATEGORY` | MaterialSampleRecord → IdentifiedConcept |
| `MSR_HAS_MATERIAL_CATEGORY` | MaterialSampleRecord → IdentifiedConcept |
| `MSR_HAS_SAMPLE_OBJECT_TYPE` | MaterialSampleRecord → IdentifiedConcept |
| `MSR_CURATION` | MaterialSampleRecord → MaterialSampleCuration |
| `MSR_RELATED_RESOURCE` | MaterialSampleRecord → SampleRelation |
| `EVENT_SAMPLING_SITE` | SamplingEvent → SamplingSite |
| `EVENT_SAMPLE_LOCATION` | SamplingEvent → GeospatialCoordLocation |
| `EVENT_RESPONSIBILITY` | SamplingEvent → Agent |
| `EVENT_HAS_CONTEXT_CATEGORY` | SamplingEvent → IdentifiedConcept |
| `SITE_LOCATION` | SamplingSite → GeospatialCoordLocation |
| `CURATION_RESPONSIBILITY` | MaterialSampleCuration → Agent |

### Two Ways to Get Geographic Coordinates
...
**Key Differences:**
- **Direct** uses `EVENT_SAMPLE_LOCATION`: 2 edges from Sample to Location
- **Site-Mediated** uses `EVENT_SAMPLING_SITE` + `SITE_LOCATION`: 3 edges from Sample to Location
...
### Full Relationship Map Using Typed Edges
...
**Edge Categories:**
- **DIRECT LOCATION**: `MSR_PRODUCED_BY` → `EVENT_SAMPLE_LOCATION`
- **SITE-MEDIATED LOCATION**: `MSR_PRODUCED_BY` → `EVENT_SAMPLING_SITE` → `SITE_LOCATION`
- **AGENT EDGES**: `MSR_REGISTRANT`, `EVENT_RESPONSIBILITY`
- **CONCEPT EDGES**: `MSR_KEYWORDS`, `MSR_HAS_*_CATEGORY`, `MSR_HAS_SAMPLE_OBJECT_TYPE`
...
## Related Samples via Direct Location (EVENT_SAMPLE_LOCATION)
...
## Related Samples via Site-Mediated Location (EVENT_SAMPLING_SITE → SITE_LOCATION)
...
1. **`sample_location_only`**: Precise field collection points (via EVENT_SAMPLE_LOCATION)
   - Most common category
   - Represents exact GPS coordinates where sampling events occurred
   - Varies per event, even within the same site

2. **`site_location_only`**: Administrative site markers (via SITE_LOCATION)
...
1. **Educational**: Makes direct vs site-mediated location distinction visually concrete
