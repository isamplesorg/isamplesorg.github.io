...
tableView = {
    ...
    function setView(mode, updateUrl) {
        ...
        if (updateUrl) setSearchParam('view', tableMode ? 'table' : null);
        if (tableMode && rows.length === 0) refreshTable();
    ...
    document.getElementById('materialFilterBody')?.addEventListener('change', () => {
        if (isTableViewActive()) refreshTable();
    });
    document.getElementById('contextFilterBody')?.addEventListener('change', () => {
        if (isTableViewActive()) refreshTable();
    });
    document.getElementById('objectTypeFilterBody')?.addEventListener('change', () => {
        if (isTableViewActive()) refreshTable();
    });
