...
Every stop is a **live link**: click it and you are standing in the view being
described, because the Explorer's URL captures its complete state.
...
Plot.plot({
  ...
  y: {label: null},
  marks: [
    Plot.barX(sources, {y: "source", x: "count", fill: "source"}),
...
folds in **place names**, so "Cyprus" finds samples whose coordinates sit on
the island even when their descriptions don't mention it. (Both were real user
...
**[Fly to central Anatolia →](https://isamples.org/explorer.html#v=1&lat=37.6619&lng=32.8334&alt=300000&mode=point)**
...
The single densest spot in the whole collection is not a volcano or a reef —
it's a Neolithic settlement mound in Turkey excavated for decades. Compare
...
<ol>${(await dense).map(d => html`<li>
  <a href="https://isamples.org/explorer.html#v=1&lat=${d.center_lat.toFixed(4)}&lng=${d.center_lng.toFixed(4)}&alt=300000&mode=point" target="_blank">
...
materials = sql(`
  ...
  LEFT JOIN read_parquet('${VOCAB_LABELS}') vl ON fs.facet_value = vl.uri
...
**Download CSV** exports exactly what you've filtered to. And because the URL
