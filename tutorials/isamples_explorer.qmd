...
function hasFacetFilters() {
    ...
    const otTotal = document.querySelectorAll('#objectTypeFilterBody input[type="checkbox"]').length;
    // Active if some (but not all) are checked, or if none are checked
    return (mat.length > 0 && mat.length < matTotal)
        || (ctx.length > 0 && ctx.length < ctxTotal)
        || (ot.length > 0 && ot.length < otTotal);
}
...
function facetFilterSQL() {
    ...
    const ot = getCheckedValues('objectTypeFilterBody');
    const otTotal = document.querySelectorAll('#objectTypeFilterBody input[type="checkbox"]').length;
    if (ot.length > 0 && ot.length < otTotal) {
        const list = ot.map(s => `'${s}'`).join(',');
        sql += ` AND f.object_type IN (${list})`;
    } else if (ot.length === 0 && otTotal > 0) {
        sql += ' AND 1=0';
    }
    return sql;
}
...
facetFilters = {
    ...
    try {
        ...
        const renderFilter = (bodyId, items) => {
            ...
            if (body && items.length > 0) {
                body.innerHTML = items.map(it =>
                    `<label><input type="checkbox" value="${it.value}" checked> ${it.value} <span style="color:#999">(${Number(it.count).toLocaleString()})</span></label>`
                ).join('');
            }
        };
        ...
        console.log(`Facet filters loaded: ${grouped.material.length} materials, ${grouped.context.length} contexts, ${grouped.object_type.length} object types`);
    } catch(err) {
...
zoomWatcher = {
    ...
    // Independent of globe mode — always shows top-N samples matching filters.
    const RESULTS_TABLE_LIMIT = 200;
    let tableReqId = 0;
    async function refreshResultsTable() {
        ...
        try {
            ...
            const note = data.length === RESULTS_TABLE_LIMIT
                ? `Showing first ${RESULTS_TABLE_LIMIT} samples matching filters (more exist).`
                : `Showing ${data.length.toLocaleString()} samples matching filters.`;
