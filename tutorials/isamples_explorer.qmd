...
zoomWatcher = {
    ...
    async function refreshFacetCountsNow(myReq) {
                ...
                if (rows && rows.length > 0) {
                    ...
                    for (const d of dims) {
                        if (d.key === singleActiveDim.key) {
                            // Selected facet keeps baseline counts (so the
                            // user can see what other values *would* yield).
        ...
        // excluding the dim being recomputed. The selected dim's own counts
        // stay at baseline (so the user can switch their selection within it).
        const queries = dims.map(async (d) => {
            if (d.key === (singleActiveDim?.key) || activeDims.find(a => a.key === d.key)) {
                // Don't recompute the column whose own values the user is
                // selecting. Show baseline so they can switch.
                applyFacetCounts(d.key, null);
                return;
            }
