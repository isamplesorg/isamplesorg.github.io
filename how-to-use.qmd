...
::: {.callout-important}
## Snapshot note (2026-08-05; live generation updated 2026-09)
...
what the Interactive Explorer serves today** — the live app reads the **`202609`**
...
1. **Open the [Interactive Explorer](/explorer.html)** — a 3D globe loads with clustered sample data
...
|---|---:|---|---:|---|
| [`current/wide.parquet`](https://data.isamples.org/current/wide.parquet) ∗ | 292 MB | Wide (one row per entity, nested relationships in `p__*` array columns) | 20.7 M | General entity queries, UI filtering, description text |
| [`isamples_202601_wide_h3.parquet`](https://data.isamples.org/isamples_202601_wide_h3.parquet) | 307 MB | Wide + H3 BIGINT indices (`h3_res4`, `h3_res6`, `h3_res8`) | 20.7 M | Geospatial queries with H3 clustering at arbitrary zoom |
| [`isamples_202512_narrow.parquet`](https://data.isamples.org/isamples_202512_narrow.parquet) | 860 MB | Narrow (graph: nodes + explicit `_edge_` rows, s/p/o/n fields) | 101,387,180 | Graph traversals, relationship-centric analysis, PQG work |
...
(292 MB, no thumbnails) is kept available for historical pinning.
...
and counts instantly, without touching the 292 MB primary file:
