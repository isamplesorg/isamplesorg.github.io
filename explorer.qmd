...
// using the viewer's current view rectangle. Returns an empty string when
// the camera can't produce a rectangle (off-globe; rare) — callers should
// treat that as "no bbox filter", matching the area-search fallback.
//
// Dateline crossing: when west > east the rectangle wraps the antimeridian;
// emit two longitude ranges OR'd together.
//
// Used by:
// - tableView (#218 follow-up): the samples table now scopes to the viewport
//   so "153 samples in view" in cluster/point mode == 153 rows in the table.
// - doSearch('area'): the "Search Selected Areas" button (#178 light path).
function viewerBboxSQL(latCol, lngCol) {
...
tableView = {
    ...
    async function refreshAll() {
        ...
        const bboxSQL = viewerBboxSQL('latitude', 'longitude');
        setLoading(true);
