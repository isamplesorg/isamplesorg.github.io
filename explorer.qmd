...
<style>
    ...
    /* Busy indicator over the map while a filter/search/facet update is in
       flight. `progress` (arrow + spinner), NOT `wait` (hourglass): the map
       stays fully interactive — we're signaling "updating", not "frozen". The
       ...
       commit paths. Cesium sets its own cursor on the canvas during camera
       interaction, so override with !important. (Pan/zoom viewport reloads run
       in separate cells that don't touch this class yet — see follow-up.) */
    body.explorer-busy #cesiumContainer,
    body.explorer-busy #cesiumContainer canvas {
