...
function desiredClusterSig() {
    return JSON.stringify({
        kind: wantFilteredClusters() ? 'filtered' : 'summary',
...
phase1 = {
    ...
    // #300: phase1 loads the pre-aggregated summary (facet-blind) clusters, so
    // label the cache with the current summary/source/tree signature. Without
    // this seed, the first post-boot cluster reconcile couldn't tell whether the
    // initial layer already matched the desired filter state (Codex P1.3).
    viewer._clusterFilterSig = desiredClusterSig();
...
zoomWatcher = {
    ...
    async function reconcileGlobeForFilters() {
        ...
        if (computeTargetMode(h) === 'point') {
            ...
            // newer loadRes of its own (Codex P1.4).
            loadResGen++;
    ...
    async function handleFacetFilterChange() {
        ...
        try {
            ...
            // Guarded by a freshness token so a rapid second toggle wins.
            const isStale = freshSelectionToken(viewer);
            const sel = viewer._globeState;
            if (sel.selectedH3) {
                const meta = await fetchClusterByH3(sel.selectedH3);
    ...
    setTimeout(() => {
        if (window.__filteredClustersReady === true && hasFacetFilters()) {
            const h = viewer.camera.positionCartographic.height;
            if (computeTargetMode(h) !== getMode()) reconcileGlobeForFilters();
        }
    }, 0);
