...
viewer = {
    ...
    // Test hook (#172, Codex round-3 P2): faithfully replay a result-pin CLICK
    // by index — the SAME ceremony the on-globe LEFT_CLICK handler runs (shared
    // selection helper + pid hash push), so specs can exercise pin selection and
    // Back-navigation without Cesium canvas picking (not feasible in Playwright).
    // Returns false if there is no pin at that index.
    if (typeof window !== 'undefined') window.__clickSearchPin = (i = 0) => {
        const col = v.searchResultPoints;
        if (!col || i < 0 || i >= col.length) return false;
        const meta = col.get(i).id;
        const isStale = freshSelectionToken(v);
        if (v.selectSearchResult) v.selectSearchResult(meta, { fly: true, isStale });
    ...
    new Cesium.ScreenSpaceEventHandler(v.scene.canvas).setInputAction(async (e) => {
        ...
        if (typeof meta === 'object' && meta.type === 'searchResult') {
            ...
            // #172 Inc 1 (Codex round-1 P1.2, snapshot semantics): this click
            // tears down the rendered results list, so the result-pin overlay
            // (its snapshot) goes with it.
            ...
            try {
                ...
                if (isStale()) return;
                // #172 Inc 1 (Codex round-3 P1): write the list + pins as ONE
                // owned operation. Re-clear here (post-await, ownership just
                // confirmed by isStale) so a search that populated pins DURING
                // this await can't leave them orphaned over these nearby rows.
                if (v.searchResultPoints) v.searchResultPoints.removeAll();
...
zoomWatcher = {
            ...
            if (!isStale()) {
                ...
                if (sel.selectedH3) {
                    ...
                    if (!meta) {
                        ...
                        const sampEl = document.getElementById('samplesSection');
                        if (sampEl) sampEl.innerHTML = '';
                        // #172 Inc 1 (Codex round-2 P1): list teardown → clear the pin snapshot too.
                    ...
                    if (!stillVisible || stillVisible.length === 0) {
                        ...
                        const sampEl = document.getElementById('samplesSection');
                        if (sampEl) sampEl.innerHTML = '';
                        // #172 Inc 1 (Codex round-2 P1): source filter invalidated the
                        // selected search result and cleared the list → clear its pins too.
                        if (viewer.searchResultPoints) viewer.searchResultPoints.removeAll();
                        writeGlobeHash(viewer, { force: true });
                    }
                }
    ...
    async function hydrateClusterUI(meta, isStale) {
        ...
        try {
            ...
            // #172 Inc 1 (Codex round-3 P1): list + pins as ONE owned operation.
            // Re-clear post-await (ownership just confirmed) so a search that
            // populated pins during this await can't orphan them over the
            // nearby rows we're about to render.
            if (viewer.searchResultPoints) viewer.searchResultPoints.removeAll();
            updateSamples(samples);
        } catch(err) {
            if (isStale && isStale()) return;
    ...
    window.addEventListener('hashchange', async () => {
        ...
        if (state.pid) {
            ...
            const sampEl = document.getElementById('samplesSection');
            if (sampEl) sampEl.innerHTML = '';
            // #172 Inc 1 (Codex round-2 P1): the Back-after-pin-click no-selection
            // branch tears down the preserved search list → clear its pins too.
            if (viewer.searchResultPoints) viewer.searchResultPoints.removeAll();
        }
    });
    ...
    async function clearActiveSearchFilter() {
        ...
        // #172 pin-overlay Inc 1 (option C): this is the committed clear used by
        // the concept-chip clear button (its only caller today) — it tears down
        // the results list, so clear the LIST and the result-pin overlay snapshot
        // TOGETHER.
        if (viewer.searchResultPoints) viewer.searchResultPoints.removeAll();
    ...
    async function doSearch(scope) {
        ...
        // #172 Inc 1 (Codex round-3 P1): take PANEL OWNERSHIP. A committed search
        // is a new panel owner, so bump the selection freshness token to
        // invalidate any in-flight selection hydration (cluster nearby-samples,
        // sample/pid detail). Those producers re-check isStale() after their
        // await and bail before overwriting this search's list/pins, so the two
        // can't diverge. We deliberately DON'T capture the token here: doSearch's
        // own post-await renders are (and stay) guarded by searchId/_searchSeq —
        // the two token systems guard different things (search build vs selection
        // panel) and both must hold. The reverse race (a cluster click AFTER this
        // submit) is handled by the owned post-await clears in the cluster-click /
        // hydrateClusterUI paths.
        freshSelectionToken(viewer);

        // #172 pin-overlay Inc 1 (option C): clear the previous result-pin
        // overlay on every committed submit (before repopulating). This covers
        // both the new-search path (repopulated below on success) and the
        // empty/too-short committed-clear path. Draft input edits never reach
        // doSearch, so typing/backspace does not clear pins.
        if (viewer.searchResultPoints) viewer.searchResultPoints.removeAll();

        const term = searchInput.value.trim();
        if (!term || term.length < 2) {
            searchResults.textContent = 'Type at least 2 characters';
            // #172 Inc 1 (Codex round-3 P1): clear the results LIST here too so
            // list and pins clear TOGETHER — otherwise stale search rows would
            // survive under an empty overlay, breaking the snapshot invariant.
            const sampElClear = document.getElementById('samplesSection');
            if (sampElClear) sampElClear.innerHTML = '';
    ...
    async function doDescribedBy(uri) {
        ...
        // #172 Inc 1 (Codex round-3 P1): take PANEL OWNERSHIP — same as doSearch.
        // A committed concept filter is a new panel owner; bump the selection
        // freshness token so any in-flight selection hydration bails at its
        // post-await isStale() check rather than overwriting this render. The
        // concept render itself stays guarded by searchId/_searchSeq.
        freshSelectionToken(viewer);

        // #172 pin-overlay Inc 1 (option C): committing a concept filter removes
        // `search=` and owns the panel, so drop any lingering text-search
        // result pins (a committed clear path for the text overlay).
        if (viewer.searchResultPoints) viewer.searchResultPoints.removeAll();
