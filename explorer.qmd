...
zoomWatcher = {
    ...
    function exitPointMode(pushHistory) {
        ...
        console.log('[A1dbg] Exited point mode (searchActive=' + searchIsActive() + ')', new Error().stack?.split('\n').slice(1,4).join(' | '));
    }
    ...
    async function applySearchFilterChange() {
        console.log('[A1dbg] applySearchFilterChange entry: active=', searchIsActive(), 'mode=', getMode());
        busyAcquire();
        try {
            syncFacetNote();
            refreshHeatmap();
    ...
    async function doSearch(scope) {
        ...
        console.log('[A1dbg] post-build: searchId=', searchId, '_searchSeq=', _searchSeq, 'active=', window.__searchFilter?.active);
