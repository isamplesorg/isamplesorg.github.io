...
nodeBitsReady = {
    try {
        const rows = await db.query(
            `SELECT facet_type, concept_uri, bit_index FROM read_parquet('${node_bits_url}')`);
        ...
        // Only advertise readiness if every tree dim actually got bits.
        const ok = ['material', 'context', 'object_type'].every(k => map[k].size > 0);
        window.__nodeBits = ok ? map : null;
        return ok;
    } catch (err) {
        console.warn('node_bits load failed; facetFilterSQL will use the membership fallback:', err);
