...
viewer = {
    ...
    // #172 Inc 1 (Codex round-4 P1): panel/list ownership generation. Every
    // #samplesSection list producer claims ++v._panelGen when it takes the list;
    // async producers write list+pins only while they still hold the latest
    // generation (see panelWriteAllowed). Closes the search-vs-cluster race in
    // BOTH orders. The pid deep-link hydration writes only the card, so it never
    // bumps this — that's why the boot search+pid list still renders.
    v._panelGen = 0;
...
zoomWatcher = {
    ...
    async function doSearch(scope) {
        ...
        freshSelectionToken(viewer);
        // (round-4, _panelGen) claim a fresh LIST generation and CAPTURE it. Every
        // post-await list write below re-checks it (in addition to _searchSeq), so
        // a newer list producer that started AFTER this search — e.g. a cluster
        // click while the search is still building — makes this render bail
        // instead of overwriting the cluster's rows. This closes the reverse race
        // order. It does NOT break boot search+pid: the pid deep-link writes only
        // the card and never bumps _panelGen, so this render still owns the list.
        const myPanelGen = ++viewer._panelGen;
    ...
    async function doDescribedBy(uri) {
        ...
        // _panelGen) claim + capture a LIST generation, re-checked before this
        // render, so a cluster click that starts AFTER this concept commit wins
        // the list instead of being overwritten. The concept render also stays
        // guarded by searchId/_searchSeq.
        freshSelectionToken(viewer);
        const myPanelGen = ++viewer._panelGen;
