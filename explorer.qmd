...
tableView = {
        ...
        if (visible.length === 0) {
            ...
            tableEl.querySelectorAll('tbody tr[data-pid]').forEach(tr => {
                tr.addEventListener('click', async (e) => {
                    ...
                    const sample = pid ? rowsByPid.get(pid) : null;
                    ...
                    // _suppressHashWrite gate could swallow the write. Calling
                    // buildHash() unconditionally keeps the URL canonical with
                    // the selection regardless of init ordering.
    ...
    async function refreshTable() {
        ...
        try {
            ...
            rows = arr;
            rowsByPid = new Map(arr.map(r => [r.pid, r]));
            renderTable();
    ...
    // Table is permanent now — load on boot instead of waiting for a
    // setView('table') call that no longer exists (issue #200 / M-5).
    refreshTable();
