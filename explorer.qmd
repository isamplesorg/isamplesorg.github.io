...
viewer = {
    ...
    // Test hook (#172): lets Playwright assert the live pin count without
    // reaching into Cesium internals.
    if (typeof window !== 'undefined') window.__searchPins = () => v.searchResultPoints.length;
    ...
    new Cesium.ScreenSpaceEventHandler(v.scene.canvas).setInputAction((movement) => {
        ...
        if (Cesium.defined(picked) && picked.primitive && picked.id) {
            ...
            if (typeof meta === 'object' && (meta.type === 'sample' || meta.type === 'searchResult')) {
                // #172 pin-overlay Inc 1: result pins hover like sample points.
    ...
    new Cesium.ScreenSpaceEventHandler(v.scene.canvas).setInputAction(async (e) => {
        ...
        if (typeof meta === 'object' && meta.type === 'searchResult') {
            ...
            // selection state, flight, lazy detail) via the shared helper, but
            // PRESERVES the #samplesSection search-results list the user is
            // browsing. Like the sample branch it also opens the floating in-map
            // card at the click pixel and writes the pid selection hash.
            if (v.selectSearchResult) {
                v.selectSearchResult(meta, { fly: true, inMapCardAt: { x: e.position.x, y: e.position.y } });
            }
...
zoomWatcher = {
    ...
    async function clearActiveSearchFilter() {
        ...
        _searchSeq++;
        // #172 pin-overlay Inc 1 (option C): committed clear (concept-chip clear
        // / programmatic ?search= removal) drops the result-pin overlay too.
    ...
    // place_name, result_time }); coord-less rows are ignored. `inMapCardAt`
    // (pin click only) additionally opens the floating in-map card at the click
    // pixel, parity with the sample-click branch.
    async function selectSearchResult(sample, { fly = true, inMapCardAt = null } = {}) {
        ...
        if (inMapCardAt) showInMapCard(sample, inMapCardAt);
        if (fly) {
            viewer.camera.flyTo({
        ...
        // Lazy-load full description (mirrors the globe click handler). Don't
        // clear samplesSection — it holds the results the user is browsing.
        try {
            const detail = await db.query(`
                SELECT description
