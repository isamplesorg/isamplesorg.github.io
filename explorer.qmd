  ...
  html:
    include-in-header:
      text: |
        ...
        <script>
          // Boot-critical preload hints for the DEFAULT data origin. Injected rather
          // than static <link> tags so a data-origin override (?data_base= or
          // localStorage ISAMPLES_DATA_BASE, same sources as the R2_BASE cell) does
          // not fire wasted prefetches that 404 against the default origin — seen
          // when previewing a release from a staging prefix. Any override skips the
          // hints entirely: that is a preview path where prefetch speed is moot, and
          // it avoids duplicating R2_BASE's override validation here. Keep these
          // filenames in lockstep with the *_url cells; the #334 manifest check
          // audits whatever preload links end up in the DOM.
          (function () {
            var override = null;
            try {
              override = new URLSearchParams(location.search).get('data_base')
                || localStorage.getItem('ISAMPLES_DATA_BASE');
            } catch (e) { /* storage blocked: treat as no override */ }
            if (override) return;
            ['isamples_202609_h3_summary_res4.parquet',
             'isamples_202609_facet_summaries.parquet',
             'vocab_labels_202609.parquet'].forEach(function (f) {
              var l = document.createElement('link');
              l.rel = 'preload'; l.as = 'fetch'; l.crossOrigin = 'anonymous';
              l.href = 'https://data.isamples.org/' + f;
...
<style>
    ...
    /* #324: in this vertical toolbar the sceneModePicker's EXPANDED
       2D/Columbus buttons drop DOWN into the baseLayerPicker button's
       space; without a stacking context the later sibling paints on top
       and silently eats the clicks (Andrea: "clicked the globe button,
       nothing happened" — elementFromPoint at the 2D button's center
       returned .cesium-baseLayerPicker-selected). Same fix family as the
       dropdown rule above. */
...
h3_res4_url = `${R2_BASE}/isamples_202609_h3_summary_res4.parquet`
h3_res6_url = `${R2_BASE}/isamples_202609_h3_summary_res6.parquet`
h3_res8_url = `${R2_BASE}/isamples_202609_h3_summary_res8.parquet`
...
// so this _v2 suffix is a one-off retrofit for 202608. (202609 kept the _v3
// filename as-is rather than renaming at the generation boundary.)
//
...
lite_url = `${R2_BASE}/isamples_202609_samples_map_lite_v3.parquet`
...
wide_url = `${R2_BASE}/isamples_202609_wide.parquet`
// v2 carries object_type alongside material and context (URI-string columns).
facets_url = `${R2_BASE}/isamples_202609_sample_facets_v4.parquet`
facet_summaries_url = `${R2_BASE}/isamples_202609_facet_summaries.parquet`
// Pre-aggregated single-filter cache for fast cross-filtered facet counts.
cross_filter_url = `${R2_BASE}/isamples_202609_facet_cross_filter.parquet`
...
vocab_labels_url = `${R2_BASE}/vocab_labels_202609.parquet`
...
facet_tree_url = `${R2_BASE}/isamples_202609_facet_tree_summaries.parquet`
membership_url = `${R2_BASE}/isamples_202609_sample_facet_membership.parquet`
...
tree_cross_filter_url = `${R2_BASE}/isamples_202609_facet_tree_cross_filter.parquet`
...
masks_url = `${R2_BASE}/isamples_202609_sample_facet_masks.parquet`
node_bits_url = `${R2_BASE}/isamples_202609_facet_node_bits.parquet`
...
// filters).
index_url = `${R2_BASE}/isamples_202609_sample_facet_index.parquet`
...
index_meta_url = `${R2_BASE}/isamples_202609_sample_facet_index_meta.parquet`
...
search_index_base = `${R2_BASE}/isamples_202609_search_index_v1`

// === #334 v0-detect: release-manifest boot cross-check ===
// The release manifest (isamples_202609_release_manifest.json, committed in
...
// the page pins against that list and complains LOUDLY on divergence —
...
manifestCheck = {
  ...
  try {
    const resp = await fetch(new URL('isamples_202609_release_manifest.json', document.baseURI));
    if (!resp.ok) {
      console.warn(`#334 manifest check: manifest fetch HTTP ${resp.status} — skipping`);
      ...
      if (result.missing.length) {
        console.error('#334 RELEASE-MANIFEST MISMATCH — the page pins data files '
          + 'absent from the release manifest (stale manifest, or an unvetted '
          + 'suffix bump — see CANONICAL.md policy):', result.missing);
        ...
        banner.textContent = 'Data release could not be validated — some data files '
        ...
        console.info(`#334 manifest check ok — ${pinnedNames.length} pinned URLs `
...
// Coherence audit (2026-08-04), structural finding: the honesty rule was
// call-site discipline, not an invariant. #340's whole mechanism was that
// "in flight" was conveyed by OPACITY ALONE — this function adds a class and
// leaves the old numeric text intact — so a recompute that never settled left
// confident, precise, wrong numbers on screen forever with no signal.
//
// Fix: after a short grace period, an unsettled count stops asserting a number
// and says "(Loading…)" instead. The grace period matters — this fires on every
// camera moveStart, and swapping text immediately would make counts flicker on
// every small pan.
//
// Be clear about what 400 ms does and does not buy (Codex review): the timer
// starts at moveStart, but the actual work waits for moveEnd plus the 250 ms
// refreshFacetCounts debounce. So on an animated flight longer than ~150 ms,
// "(Loading…)" appears BEFORE the query even starts, and a one-second pan shows
// it for most of the flight. That is honest — the counts really are unknown
// mid-flight — but it is emphatically not limited to pathological stalls. The
// window only suppresses the flicker on short, snappy interactions.
//
// This is deliberately NOT a watchdog that forces a terminal state: per the
// audit's structural recommendation, honesty only requires that invalid data
// stop LOOKING current. A persistent "(Loading…)" is honest. Making the work
// actually terminate (and cancelling the underlying DuckDB query) is separate,
// larger, post-grant work.
// NB: state lives on `window`, not a top-level `let`. Quarto {ojs} cells reject
// top-level const/let/var — each top-level statement is a reactive cell, and a
// bad declaration cascades and breaks every dependent cell (i.e. the whole
// Explorer). `window.__*` is the established pattern in this file for mutable
// cross-cell state (cf. window.__searchFilter, window.__facetIndexStatus).
function facetCountsStaleDelayMs() { return 400; }

function clearFacetCountsStaleTimer() {
    if (typeof window === 'undefined') return;
    if (window.__facetCountsStaleTimer) {
        clearTimeout(window.__facetCountsStaleTimer);
        window.__facetCountsStaleTimer = null;
    }
}

function markFacetCountsRecomputing() {
    document.querySelectorAll('.facet-count').forEach(el => el.classList.add('recomputing'));
    clearFacetCountsStaleTimer();
    window.__facetCountsStaleTimer = setTimeout(() => {
        window.__facetCountsStaleTimer = null;
        // Self-filtering: applyFacetCounts removes `.recomputing` as each dim
        // settles, so anything still carrying it is genuinely unresolved. No
        // per-dimension bookkeeping needed, and a partially-settled repaint
        // cannot be clobbered.
        const inFlight = Array.from(document.querySelectorAll('.facet-count.recomputing'));
        if (!inFlight.length) return;
        const dims = new Set();
        for (const el of inFlight) {
            // Never overwrite the honest dash (e.g. counts unavailable under an
            // active search, #340) with a "still loading" claim — that would be
            // a downgrade from "we can't know this" to "we're about to know it".
            if (el.classList.contains('count-unavailable')) continue;
            el.textContent = '(Loading…)';
            el.removeAttribute('title');
            const k = el.getAttribute('data-facet');
            if (k) dims.add(k);
        }
        // Unknown must not imply zero: drop stale `.zero` dimming on exactly
        // the dims we just marked unknown, and no others.
        for (const k of dims) clearStaleZeroRows(k);
    }, facetCountsStaleDelayMs());
}
...
function markFacetCountsUnavailable(facetKeys = null) {
    // Terminal state: cancel any pending stale-swap so it can't run and
    // (harmlessly, but confusingly) re-touch elements after we've settled.
    if (facetKeys === null) clearFacetCountsStaleTimer();
...
function markFacetCountsPending(facetKeys = null) {
    // Already the "still loading" text the stale-swap would produce; drop the
    // timer so it doesn't fire redundantly.
    if (facetKeys === null) clearFacetCountsStaleTimer();
    const keys = facetKeys || ['source', 'material', 'context', 'object_type'];
    for (const key of keys) {
        document.querySelectorAll(`.facet-count[data-facet="${key}"]`).forEach(el => {
...
db = {
    ...
    // settle (buffer registered, or fetch failed → keep the URL and range-read
    ...
    //
    // Liveness (Codex rounds 1-2, 5): the shared liteReady gate is bounded —
    ...
    // this gate — the query's own execution/ranged GETs have no deadline, as
    // before.
    let resolveLite;
    ...
    // were released at the same instant — Codex round-4 P1). Same cap.
    ...
    // URL — they read the footer only and must not wait for the 63 MB fetch.
...
// traffic 156 MB → 86 MB, and the samples table appeared at 63 s instead of
...
// worker copies it into the heap). On any failure — HTTP error, short read,
// or no bytes for STALL_MS (the watchdog below aborts a stalled fetch) —
...
liteFile = {
    ...
    try {
        ...
        // (Not a content check — a smaller *valid* parquet under this URL
...
// sample_facets_v4.parquet's material / context / object_type columns).
...
zoomWatcher = {
    ...
    async function loadViewportSamples() {
        ...
        try {
            ...
            // Coherence audit F5: when the cap is hit we do NOT know the in-view
            // total until the COUNT below succeeds. If it fails we must say so
            // rather than pass the cap off as the total (see below).
            let totalCountKnown = true;
            if (data.length >= POINT_BUDGET) {
                try {
                    ...
                    // Don't fail the whole load if the count query fails — but
                    // do NOT pretend the render cap is the true total. We hit
                    // POINT_BUDGET, so the real number is >= data.length and
                    // could be 10-100x higher. Reporting "5,000 samples in
                    // view" here is simply false (coherence audit F5; same bug
                    // shape #206 fixed for the no-failure case).
                    totalCountKnown = false;
                    console.warn("Real-count query failed; in-view total is unknown:", err);
                }
            }
            ...
            // F5: '—' when the COUNT failed — the cap was hit, so the true
            // in-view total is unknown and definitely NOT samples.length.
            updateStats('Samples', samples.length, totalCountKnown ? totalCount : '—', `${(elapsed/1000).toFixed(1)}s`, 'Samples Rendered', 'Samples in View');
            const phaseMsg = !totalCountKnown
                ? `Showing ${samples.length.toLocaleString()} samples (display limit reached — total in view unavailable). Click one for details.`
                : capReached
            ...
            // F5: diagnostics must not assert the cap as the total either — a
            // dishonest debug log is how a future reader mis-diagnoses this.
            window.a1dbg?.('point-load-render', { rendered: samples.length, totalCount: totalCountKnown ? totalCount : null, totalCountKnown, searchActive: searchIsActive(), searchFiltered: !!searchFilterSQL('pid') });
            console.log(`Point mode: rendered ${samples.length} of ${totalCountKnown ? totalCount : 'unknown'} samples in ${elapsed.toFixed(0)}ms${capReached ? ' (cap reached)' : ''}${totalCountKnown ? '' : ' (count query failed)'}`);
    ...
    async function updateCrossFilteredCounts(myReq) {
        ...
        await Promise.all(dims.map(async (d) => {
            try {
                ...
                // Coherence audit F3 / PLAN_305 Phase 4 ("remove the misleading
                // active-filter baseline fallback everywhere"). The old Q3
                // behavior painted applyFacetCounts(d.key, null) — the
                // UNFILTERED global baseline — when this query threw. Under
                // active filters that is a confidently wrong number with no
                // signal: the repaint even clears `.recomputing`, so it looks
                // settled. That is the #304/#340 failure mode in miniature.
                // Honest dash instead. (Under NO active constraint the baseline
                // would have been correct, but this catch is only reachable on
                // the constrained path, so the dash is the safe answer.)
                markFacetCountsUnavailable([d.key]);
            }
        }));
    }
    ...
    function reconcileSettledCamera(v) {
        ...
        if (getMode() === 'cluster' && v._clusterData) {
            // Coherence audit M2 + Codex review. Counting `_clusterData` says
            // nothing about whether that cache is CURRENT. It can be a previous
            // filter's cells, a previous resolution's cells, or the facet-blind
            // summary cache, while a reload is in flight or has just failed.
            // Painting from it would replace an honest "Loading H3…" or
            // "Failed to load…" with a green settled count — turning a visible
            // problem into a confident wrong number, which is precisely the
            // failure class this audit exists to remove. So: only speak when
            // the cache is provably current; otherwise stay quiet and let the
            // loading/failure message stand.
            const alt = v.camera.positionCartographic.height;
            const targetRes = alt > 3000000 ? 4 : alt > 300000 ? 6 : 8;
            const cacheIsCurrent = !loading
                && computeTargetMode(alt) === 'cluster'
                && currentRes === targetRes
                && v._clusterFilterSig === desiredClusterSig();
            if (cacheIsCurrent) {
                const inView = countInViewport(paddedViewportBounds(VIEWPORT_PAD_FACTOR));
                ...
                // M2 proper: exitPointMode() writes "N clusters in view" into the
                // ALWAYS-VISIBLE phase message, but this settle path previously
                // refreshed only the (collapsed-by-default) stats panel — so after
                // leaving point mode, the next ordinary pan stranded a previous
                // viewport's count on screen, stated as fact.
                updatePhaseMsg(`${inView.clusters.toLocaleString()} clusters in view. Zoom closer for individual samples.`, 'done');
            }
        }
    ...
    async function buildSearchFilter(terms, term) {
        ...
        // the small search_pids table instead of RE-scanning the 69 MB facets
    ...
    // sharded inverted index at data.isamples.org/isamples_202609_search_index_v1/
    ...
    const SUBSTRATE_BASE = search_index_base;   // shared cell — see #334 manifest check
    ...
    async function loadSubstrateMeta() {
        // Three KB-scale sidecars, fetched once per session on FIRST substrate
        // search (deliberately NOT at boot — flag-off visitors pay nothing).
        if (_substrateMeta) return _substrateMeta;
        const [hot, sizes, stats] = await Promise.all([
            fetch(`${SUBSTRATE_BASE}/hot_tokens.json`).then(r => r.json()),
            fetch(`${SUBSTRATE_BASE}/shard_sizes.json`).then(r => r.json()),
            fetch(`${SUBSTRATE_BASE}/build_stats.json`).then(r => r.json()),
        ]);
        _substrateMeta = {
            manifest: { shardCount: stats.shard_count, hotTokens: hot.tokens },
            shardSizes: sizes,
            stats: {
                totalDocs: stats.total_documents,
                avgDocLenByField: Object.fromEntries(
                    Object.entries(stats.fields).map(([f, v]) => [f, v.avg_doc_len])),
            },
        };
        return _substrateMeta;
    }

    let _substrateModPromise = null;
    ...
    async function buildSearchFilterSubstrate(term) {
        ...
        if (token !== _searchFilterToken) return false;   // superseded during meta fetch
        const plan = _searchSubstrate.planQuery(term, meta.manifest, meta.shardSizes);
        if (plan.mode === 'empty' || plan.mode === 'allStopwords') {
            // Controlled empty per contract §3 — never a corpus dump.
            await clearSearchFilter();
            window.__searchFilter = {
                active: false, term: null, token: _searchFilterToken, total: 0,
                kind: 'text', substrate: true,
                note: plan.mode === 'allStopwords'
                    ? 'All terms in your query are common words. Add a more specific term to search.'
                    : null,
            };
            return false;
        }
        // Tokens are lowercase alphanumeric BY TOKENIZER CONSTRUCTION (see
        // search_tokenizer §4: every non-\p{L}\p{N} char became a separator),
        // so interpolating them into SQL string literals cannot break out.
        const tokList = plan.tokens.map(t => `'${t}'`).join(',');
        const stats = meta.stats;
        const staging = `search_pids_next_${token}`;
        try {
            if (plan.mode === 'topk') {
                // All surviving terms are common: intersect the precomputed
                // per-token top-500 lists, sum static scores (§6 —
                // documented approximation, benchmarked at #172).
                await db.query(`
                    CREATE OR REPLACE TABLE ${staging} AS
                    WITH agg AS (
                        SELECT pid, sum(static_score) AS relevance_score,
                               count(DISTINCT token) AS m
                        FROM read_parquet('${SUBSTRATE_BASE}/hot_topk.parquet')
                ...
                const urls = plan.files.map(f => `'${SUBSTRATE_BASE}/${f}'`).join(',');
                const avgCase = Object.entries(stats.avgDocLenByField)
                    .map(([f, v]) => `WHEN '${f}' THEN ${Number(v)}`).join(' ');
                // BM25 per contract §5 — identical constants/shape to the
                // builder's hot_topk scoring and the JS module's
                // bm25Contribution (which unit-tests pin). df is EMBEDDED in
                // the shard rows (round-5 amendment) — no df.parquet fetch.
                await db.query(`
                    CREATE OR REPLACE TABLE ${staging} AS
                    WITH hits AS (
                        SELECT token, pid, field, tf, doc_len, df
                        ...
                        SELECT token, pid,
                            (CASE field
                                ...
                                ELSE 1.0 END)
                            * ln(((${stats.totalDocs} - df + 0.5) / (df + 0.5)) + 1)
                            * (tf * 2.2) / (tf + 1.2 * (0.25 + 0.75 * doc_len /
                                (CASE field ${avgCase} ELSE 10.0 END))) AS c
                        FROM hits
                    ), agg AS (
                        SELECT pid, sum(c) AS relevance_score,
                               count(DISTINCT token) AS m
            ...
            if (token !== _searchFilterToken) return false;   // superseded
            await db.query(`CREATE OR REPLACE TABLE search_pids AS
                SELECT pid, label, source, place_name, relevance_score FROM ${staging}`);
            if (token !== _searchFilterToken) return false;
            const cnt = Array.from(await db.query(`SELECT COUNT(*) AS n FROM search_pids`));
            ...
            if (token !== _searchFilterToken) return false;
            window.__searchFilter = {
                active: true, term, token, total, kind: 'text', substrate: true,
                substrateMode: plan.mode,
    ...
    async function doSearch(scope) {
        ...
        const searchHeadingHTML = (suffix) => {
            // #171 disclosure (contract §3/§6): when the substrate path
            // dropped common terms from the AND, the user must see exactly
            // which words were ignored — silent semantic change is the one
            // thing the policy forbids.
            const sf = (typeof window !== 'undefined') ? window.__searchFilter : null;
            const ignored = (sf && sf.substrate && sf.ignoredCommon && sf.ignoredCommon.length)
                ? `<div style="font-size: 11px; font-weight: normal; margin-top: 2px;">Very common word${sf.ignoredCommon.length > 1 ? 's' : ''} ignored: ${sf.ignoredCommon.map(escapeHtml).join(', ')}</div>`
                : '';
            return `<h4 class="search-results-heading" style="position: sticky; top: 0; z-index: 1; margin: 0 0 6px; padding: 6px 8px; background: #e3f2fd; color: #1565c0; border-radius: 4px;">Search results: "${term}"${suffix}${ignored}</h4>`;
        };
        ...
        try {
            ...
            // Guard the clear (Codex review of #171): an error inside a
            // SUPERSEDED search's build must not blow away the newer
            // search's freshly-built filter. Applies to both paths.
            if (searchId === _searchSeq) await clearSearchFilter();
        }
        ...
        try {
            ...
            // instead of re-scanning the 69 MB facets parquet — the old
            ...
            // queries SLOWER than baseline — benchmark 2026-07-11, Codex-
            ...
            async function runWorldQuery() {
                return db.query(`
                    ${
                        ...
                        // other case — the interim producer, or no source
                        // filter — keep the original cheap shape: filter +
            ...
            if (results.length === 0) {
                ...
                // and (#171, Codex review) a substrate controlled-empty state
                // (all-stopword query) must show ITS copy, not a generic
                // "No results": the message tells the user what to do.
                const substrateNote = (window.__searchFilter?.substrate && window.__searchFilter?.note)
                    ? window.__searchFilter.note : null;
                searchResults.textContent = searchFilterBuildFailed
                    ? `Search error: couldn't build the filter for "${term}". Please try again.`
                    : (substrateNote || `No results for "${term}"`);
                ...
                if (sampEl0) sampEl0.innerHTML = searchHeadingHTML(searchFilterBuildFailed ? '' : ' (0)')
                    + `<div class="empty-state" style="padding: 8px;">${searchFilterBuildFailed ? 'Search failed to build — please try again.' : escapeHtml(substrateNote || 'No samples matched this search.')}</div>`;
                return;
            }
...
| **Click sample** | Full dataset | 237 MB (range req.) | Full metadata for 1 sample |

*Sizes are decimal MB (10⁶ bytes), matching the manifest's byte counts.*

**Static files, zero backend.** All queries run in your browser via DuckDB-WASM
with HTTP range requests — only the bytes you need are transferred. The manifest
directly inventories 21 objects — including five search-index sidecars — and
summarizes the index's 1,025 shard files (256 base plus 769 hot); the index
directory holds 1,030 objects in all. Any one view touches only a handful of them.
...
<!-- Coherence-audit follow-up (2026-08-05). Sizes are from the release manifest:
     res4 505,651 B; res6 1,322,875 B; res8 2,009,510 B; map_lite_v3 62,924,115 B;
     wide 300,303,095 B. Cluster counts verified against parquet row counts:
     38,462 / 112,019 / 176,669 (the manifest does not carry row counts).

     The previous figures (580 KB / 1.6 MB / 2.5 MB) were not invented — they are
     correct for the **202601** snapshot (verified live: 594,541 / 1,627,502 /
     2,543,412 B). This page describes the LIVE app, which serves **202608**, so it
     had silently drifted a generation behind. Beware: a "wrong number" in this repo
     may be a right number for a different release — always check which snapshot a
     doc is describing before correcting it.

     Scope of this pass (Codex review): explorer.qmd, EXPLORER_QUERIES.md and
     index.qmd describe the CURRENT app and were corrected. SERIALIZATIONS.md,
     how-to-use.qmd, data.qmd and tutorials/why_h3.qmd quote 202601 objects whose
     BYTES are correct — those were deliberately left alone. They are NOT certified
     clean, though: their surrounding present-tense prose still describes those
     202601 files as the ones serving the Interactive Explorer, which is no longer
     true. Rewording them as historical/archival is open follow-up work, not done
     here. query-spec.qmd carries deeper drift still (stale filenames AND a
     text-search narrative that predates the FTS substrate shipping as default on
     2026-07-17); it now carries its own "known drift" banner rather than a silent
     partial correction, because fixing it properly means re-stating semantics, not
     renaming strings.

     Units: decimal MB (10^6 B). The old row mixed conventions (580 KB was KiB
     while 1.6/2.5 MB were decimal), which is how it read as self-consistent.
     If you edit these, re-derive from the manifest — do not round from memory.

     Generation flip 202608 -> 202609 (2026-09). Re-derived from the 202609 release
     manifest: res4 505,630 B; res6 1,322,935 B; res8 2,009,551 B; map_lite_v3
     62,923,910 B; wide 236,995,193 B. Only the wide row's rounded figure moved
     (300 -> 237 MB): same 170 row groups and ZSTD codec as 202608, 144 fewer
     location rows; the size drop itself was not investigated further. Cluster
     counts unchanged — 38,462 / 112,019 / 176,669 — checked against the parquet
     row counts of BOTH generations. Search index: 256 base + 769 hot = 1,025
     shard files, 1,030 objects with the five sidecars (was 847 / 852). The
     manifest is deployed with the site (_quarto.yml resources), so its link is
     isamples.org, not data.isamples.org (the old link 404'd). -->

::: {.callout-tip collapse="true"}
## Check these numbers against the source

Every **file-size** figure above comes from the published release manifest, which
the Explorer also cross-checks at boot. (The cluster counts come from the parquet
row counts — see below — and the timings, the 120 km threshold and the 5K point
budget are properties of the app, not the data.) To re-derive the sizes without
taking this page's word for it:

```bash
curl -s https://isamples.org/isamples_202609_release_manifest.json \
...
SELECT count(*) FROM read_parquet(
  'https://data.isamples.org/isamples_202609_h3_summary_res4.parquet');
```
:::
