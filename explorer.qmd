...
1. **Instant** (<1s): Pre-aggregated H3 res4 summary (494 KB) → 38K colored circles
...
with HTTP range requests — only the bytes you need are transferred. The published
release is 21 objects (18 parquet + 3 small JSON search-index files); any one view
touches only a few of them. The authoritative inventory is
...
<!-- Coherence-audit follow-up (2026-08-05). Sizes are from the release manifest:
     ...
     correct for the **202601** snapshot, which SERIALIZATIONS.md, how-to-use.qmd,
     data.qmd and tutorials/why_h3.qmd still legitimately document. This page
     describes the LIVE app, which serves **202608**, so it had silently drifted a
     generation behind. Beware: a "wrong number" here may be a right number for a
     different snapshot — always check which release a doc is describing before
     correcting it.
...
Every figure above comes from the published release manifest, which the Explorer
also cross-checks at boot. To re-derive them without taking this page's word for it:
