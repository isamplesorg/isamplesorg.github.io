...
zoomWatcher = {
    ...
    async function doSearch(scope) {
        ...
        try {
            ...
            // Viewport predicate (#178 Light path). Goes on the OUTER query
            // (post-join) because lat/lng live in samples_map_lite, not
            // sample_facets_v2. Implication: the inner CTE's LIMIT 50 may
            // contain hits that fail the viewport, so area-scoped searches
            // can return < 50 results — users widen by panning.
            // Dateline-crossing: split into two longitude ranges when the
            // viewer's view rectangle wraps (west > east).
