...
db = {
    ...
    // EVERY query through a FIFO chain so at most one runs at a time. SQL queries
    // are atomic (none awaits another mid-execution), so chaining can't deadlock,
    // and the latency cost is small (boot queries are individually fast). This is
    // the single-point fix; without it #300's filtered clusters never load.
...
zoomWatcher = {
    ...
    window.__onFilteredClustersReady = () => {
        if (hasFacetFilters() && !loading) reconcileGlobeForFilters();
    };
    setTimeout(() => {
        if (window.__filteredClustersReady === true && hasFacetFilters() && !loading) {
            const h = viewer.camera.positionCartographic.height;
