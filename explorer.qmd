...
manifestCheck = {
  const pinned = [
    ...
    `${R2_BASE}/isamples_202608_search_index_v1/build_stats.json`,
    `${R2_BASE}/isamples_202608_search_index_v1/hot_tokens.json`,
    `${R2_BASE}/isamples_202608_search_index_v1/shard_sizes.json`,
    `${R2_BASE}/isamples_202608_search_index_v1/df.parquet`,
  ];
  const prefix = `${R2_BASE}/`;
  const pinnedNames = pinned.map(u => u.startsWith(prefix) ? u.slice(prefix.length) : u);
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      result.missing = pinnedNames.filter(n => !manifestNames.has(n));
