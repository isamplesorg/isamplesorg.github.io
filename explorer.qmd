...
facetIndexReady = {
    ...
    try {
        ...
        // stale/partial index (fewer pids), an equal-cardinality PID replacement,
        // AND source drift, all of which a bare row-count check would miss (Codex r2).
        // It is still a cardinality/source proxy, NOT the full coverage fingerprint
        // (that's the build-time gate, SERIALIZATIONS §4.12) — a same-source-histogram
        // mask corruption could pass, which the generation-id match above guards.
